...
NDN_LOG_INIT(ndn.security.conf.KeyLocatorChecker);

bool
KeyLocatorChecker::check(const Data& data, const KeyLocator& keyLocator)
{
  return check(data.getName(), keyLocator);
}

bool
KeyLocatorChecker::check(const Interest& interest, const KeyLocator& keyLocator)
{
  if (interest.getName().size() < command_interest::MIN_SIZE) {
    NDN_LOG_DEBUG("KeyLocator check failed: interest " << interest << " has no signature");
    return false;
  }

  Name signedName = interest.getName().getPrefix(-command_interest::MIN_SIZE);
  return check(signedName, keyLocator);
}
...
bool
RelationKeyLocatorNameChecker::check(const Name& packetName, const KeyLocator& keyLocator)
{
  try {
    ...
    NDN_LOG_DEBUG("KeyLocator check failed: relation of packet " << packetName
                  << " to KeyLocator " << keyLocator.getName() << " is invalid");
    return false;
  }
  catch (const KeyLocator::Error& e) {
    NDN_LOG_DEBUG("KeyLocator check failed: packet's " << packetName << " KeyLocator is not name");
    return false;
  }
}

RegexKeyLocatorNameChecker::RegexKeyLocatorNameChecker(const Regex& regex)
  : m_regex(regex)
{
}

bool
RegexKeyLocatorNameChecker::check(const Name& packetName, const KeyLocator& keyLocator)
{
  try {
...
HyperKeyLocatorNameChecker::HyperKeyLocatorNameChecker(const std::string& pExpr, const std::string pExpand,
...
bool
HyperKeyLocatorNameChecker::check(const Name& packetName, const KeyLocator& keyLocator)
{
  try {
    if (m_hyperPRegex->match(packetName) && m_hyperKRegex->match(keyLocator.getName()) &&
        checkRelation(m_hyperRelation, m_hyperKRegex->expand(), m_hyperPRegex->expand())) {
      return true;
    }
...
KeyLocatorCheckerFactory::create(const ConfigSection& configSection, const std::string& filename)
{
