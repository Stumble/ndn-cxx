...
private:
  ...
  using CommandInterestValidationContinuation =
    std::function<void(const Interest& interest, const shared_ptr<ValidationState>& state,
  ...
  void
  checkTimestamp(const shared_ptr<CertificateRequest>& certRequest, const shared_ptr<ValidationState>& state,
                 const Name& keyName,
...
private:
  unique_ptr<ValidationPolicy> m_inner;
  Options m_options;
