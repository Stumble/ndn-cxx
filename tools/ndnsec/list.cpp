...
public:
  ...
  void
  printIdentity(const Identity& identity, bool isDefault)
  {
    if (isDefault)
      std::cout << "* ";
    else
    ...
    if (m_verboseLevel >= 1) {
      ...
      for (const Key& key : identity.getKeys()) {
  ...
  void
  printKey(const Key& key, bool isDefault)
  {
    if (isDefault)
      std::cout << "  +->* ";
    else
    ...
    if (m_verboseLevel >= 2) {
      ...
      for (const v2::Certificate& cert : key.getCertificates()) {
  ...
  void
  printCertificate(const v2::Certificate& cert, bool isDefault)
  {
    if (isDefault)
      std::cout << "       +->* ";
    else
