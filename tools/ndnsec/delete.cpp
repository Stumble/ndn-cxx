...
ndnsec_delete(int argc, char** argv)
{
  ...
  try {
    if (isDeleteCert) {
      Key key = keyChain.getPib()
        .getIdentity(v2::extractIdentityFromCertName(name))
        .getKey(v2::extractKeyNameFromCertName(name));

      keyChain.deleteCertificate(key, key.getCertificate(name).getName());
      std::cerr << "OK: Delete certificate: " << name << std::endl;
    }
    else if (isDeleteKey) {
      Identity identity = keyChain.getPib()
        .getIdentity(v2::extractIdentityFromKeyName(name));
  ...
  catch (const Tpm::Error& e) {
    std::cerr << "ERROR: Cannot delete the item: " << e.what() << std::endl;
    return 2;
  }
  catch (const v2::KeyChain::Error& e) {
    std::cerr << "ERROR: " << e.what() << std::endl;
    return 2;
  }
