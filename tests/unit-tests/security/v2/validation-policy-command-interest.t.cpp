...
public:
  CommandInterestPolicyWrapper()
    : ValidationPolicyCommandInterest(make_unique<ValidationPolicySimpleHierarchy>(), T::getOptions())
  {
  }
};

template<class T>
class ValidationPolicyCommandInterestFixture : public HierarchicalValidatorFixture<CommandInterestPolicyWrapper<T>>
{
public:
...
BOOST_AUTO_TEST_CASE(Basic)
{
  ...
  VALIDATE_FAILURE(i2, "Should fail (same timestamp)");
}
...
BOOST_AUTO_TEST_CASE(InnerReject)
{
  ...
  VALIDATE_FAILURE(i1, "Should fail (inner validator should reject)");
}
...
BOOST_FIXTURE_TEST_CASE(TimestampOutOfGraceNegative, ValidationPolicyCommandInterestFixture<GracePeriod15Sec>)
{
  ...
  VALIDATE_FAILURE(i1, "Should fail (timestamp outside the grace period)");
  systemClock->advance(time::seconds(-50)); // compensating clock change inside the VALIDATE_FAILURE
                                 ...
                                 ValidationPolicyCommandInterestFixture<GracePeriod>)
{
  ...
  this->systemClock->advance(time::seconds(-50)); // compensating clock change inside the VALIDATE_SUCCESS

  this->advanceClocks(time::milliseconds(1));
  VALIDATE_FAILURE(i2, "Should fail when validating at 1ms");
}

class TimestampsLimitedOptions
{
public:
  static ValidationPolicyCommandInterest::Options
  getOptions()
...
BOOST_FIXTURE_TEST_CASE(TimestampsLimited, ValidationPolicyCommandInterestFixture<TimestampsLimitedOptions>)
{
...
class TimestampsUnlimitedOptions
{
public:
  static ValidationPolicyCommandInterest::Options
  getOptions()
...
BOOST_FIXTURE_TEST_CASE(TimestampsUnlimited, ValidationPolicyCommandInterestFixture<TimestampsUnlimitedOptions>)
{
...
class TimestampsDisabledOptions
{
public:
  static ValidationPolicyCommandInterest::Options
  getOptions()
...
BOOST_FIXTURE_TEST_CASE(TimestampsDisabled, ValidationPolicyCommandInterestFixture<TimestampsDisabledOptions>)
{
  auto i1 = makeCommandInterest(identity); // signed at 0s
  advanceClocks(time::seconds(1));
  auto i2 = makeCommandInterest(identity); // signed at +1s
...
BOOST_FIXTURE_TEST_CASE(TtlLimited, ValidationPolicyCommandInterestFixture<TtlLimitedOptions>)
{
  auto i1 = makeCommandInterest(identity); // signed at 0s
...
BOOST_FIXTURE_TEST_CASE(TtlZero, ValidationPolicyCommandInterestFixture<TtlZeroOptions>)
{
  auto i1 = makeCommandInterest(identity); // signed at 0s
  advanceClocks(time::seconds(1));
  auto i2 = makeCommandInterest(identity); // signed at +1s
